...
export default class Table {
     ...
     * @param renderHeader
     */
    constructor({ outer, headers=[], renderHeader, dataSource, renderTr=()=>{}, renderTrs, pagination={show:false}, storage=false, loading=true, singleTr=true, emptyMsg='暂无数据', afterRender=()=>{}, sorter=()=>{}, renderEmpty }) {
        this.outer = outer;
        ...
        if(!this.renderHeader && !this.headers.length) throw new Error('Table param header must be an Array');
    ...
    _getHtml(thead){
        return `<div class="ys-table-scroll-c"><table class="ys-table${this.singleTr?' single-tr':''}">
    <thead>
    ${thead}
    </thead>
    <tbody>
    </tbody>
</table></div><div class="ys-table-no-msg-c">${this._getNomsgHtml()}</div>${this._getLoadingHtml()}${this.pagination.show?`<div class="ys-paginar"></div>`:``}`;
    }
    ...
    _getThead(){
        if(typeof this.renderHeader === 'function') return this.renderHeader();
        return `<tr>

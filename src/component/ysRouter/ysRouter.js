...
export default class YsRouter {
    ...
    push({state={},title='',url='',page}){
        if(url === window.basePath+'index.html' || url === window.basePath+'index') url = window.basePath;
        let route;
        if(page){
        ...
        if(!route) {
            return this._jumpToError(404,url);
        }
        if(route.reject){
            return this._jumpToError(403,url);
        }
        ...
        this.outer.children().hide();
        this.currentRoute.page.init(this.currentRoute,this,'push',this.outer);
    }
    ...
    replace({state={},title='',url='',page}){
        if(url === window.basePath+'index.html') url = window.basePath;
        let route;
        if(page){
            route = this.getRouteByName(page);
            url = route.path;
        ...
        this.currentRoute = route;
        this.outer.children().hide();
        this.currentRoute.page.init(this.currentRoute,this,'replace',this.outer);
    }

    _jumpToError =(code,url)=> {
        this.currentRoute = this.getRouteByName('error');
        this.currentRoute.state = {
            code
        };
        this.history.replaceState({ code },'',url);
        this.outer.children().hide();
        this.currentRoute.page.init(this.currentRoute,this,'replace',this.outer);
    };
    ...
    initCurrent(){
        ...
        if(path === window.basePath+'index.html') path = window.basePath;
        const route = this._findRouteByPath(path);
        if(!route) {
            return this._jumpToError(404,path);
        }
        if(route.reject){
            return this._jumpToError(403,path);
        }

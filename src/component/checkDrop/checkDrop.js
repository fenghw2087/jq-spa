...
export default class CheckDrop {
     ...
     * @param pickAll 全选
     */
    constructor({ obj, renderLi=row=>row, list=[], placeholder='不限', checkIds='', onSelectChange=()=>{}, pickAll }){
...
<div class="flexbox aic">
    ...
    getChecked =()=> {
        ...
        this.valueC.text(checkIds.length?this.renderLi(this.list[checkIds[0]],checkIds[0])+'等'+ checkIds.length +'项':this.placeholder).toggleClass('has-value',checkIds.length>0);
        return checkIds;
    };

    getCheckedItem =()=> {
        return this.list.filter((v,i)=>!!this.checkDic[i]);
    };

    setIndex =(indexs)=> {
        this.valueC.text(indexs.length?this.renderLi(this.list[indexs[0]],indexs[0])+'等'+ indexs.length +'项':this.placeholder).toggleClass('has-value',indexs.length>0);
        this.checkDic = this.checkDic.map((v,i)=>{
            return indexs.indexOf(i)>-1
        });
        [].forEach.call(this.checkItems,(v,i)=>{
            this.checkItems.eq(i).toggleClass('checked',this.checkDic[i-1]);
        });
        this.checkItems.eq(0).toggleClass('checked',!indexs.length);
    };
    ...
    _bindEvent =()=> {
        ...
        this.outer.on('click','.ys-check-drop-btn',function () {
            ...
            that.onSelectChange(that.getChecked(),that.getCheckedItem());
            that.toggleDrop('close');
        }).on('click','.ys-check-drop-abtn',function () {
            [].forEach.call(that.checkItems,(v,i)=>{
                that.checkItems.eq(i).addClass('checked');
                that.checkDic[i] = true;
            });
            that.checkItems.eq(0).removeClass('checked');
            that.onSelectChange(that.getChecked(),that.getCheckedItem());
            that.toggleDrop('close');
        });
